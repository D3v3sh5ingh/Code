...
# page.text contains the html or the page source
soup = BeautifulSoup(page.text, 'html.parser')
contest_problem_url = 'https://codeforces.com/contest/{}/problem/'.format(
    contest_id)
...
def create_problem_folder(prob_no, prob_name, contest_problem_url, folder_name, extension, template_txt, contest_id):
    ...
    get_contest_io(contest_problem_url, prob_folder_name,
                   prob_no, prob_name, contest_id)
...
def get_folder_name(contest_name):
    ...
    for str_char in illegal:
        folder_name = folder_name[0:10] + \
            folder_name[10:].replace(str_char, " ")
    return folder_name
...
def standings_row_extraction(contest_name, folder_name, prob_x):
    ...
    acc_tried_AC = acc_tried.findAll(
        'span', attrs={"class": "cell-passed-system-test"})
    ...
    for i in range(len(acc_tried_AC)):
        ...
        temp_text = "\nProblem {} - Accepted: {} Tried: {} Success: {}%".format(
            i, ac_num, try_num, per)
        acc_y.append(per)
...
problems = soup.find('div', attrs={"class": "datatable"}).find(
    'table').findAll('a')
...
for i in range(len(problems)):
    ...
    create_problem_folder(prob_no, prob_name, contest_problem_url,
                          folder_name, extension, template_txt, contest_id)
